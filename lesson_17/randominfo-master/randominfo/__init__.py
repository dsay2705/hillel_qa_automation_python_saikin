...
def get_country(first_name=None):
    generated_countries = [
        "USA", "Canada", "Germany", "France", "Ukraine",
        "Poland", "Japan", "Spain", "UK"
    ]
    countryFile = csv.reader(open(full_path('data.csv'), 'r', encoding='utf-8'))
    rows = list(countryFile)
    header = [h.strip().lower() for h in rows[0]]
    data_rows = rows[1:]

    if "country" not in header:
        return choice(generated_countries)

    country_index = header.index("country")
    if first_name is not None:
        for row in data_rows:
            if len(row) > country_index and row[0].strip() == first_name:
                country = row[country_index].strip()
                if country:
                    return country

    country_list = [
        row[country_index].strip()
        for row in data_rows
        if len(row) > country_index and row[country_index].strip()
    ]
    return choice(country_list) if country_list else choice(generated_countries)
